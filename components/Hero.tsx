'use client'

import Link from 'next/link'
import { useState } from 'react'

export default function Hero() {
  const [open, setOpen] = useState(false)

  return (
    <section className="flex items-start pt-24 md:pt-32 px-6 md:px-16">
      <div className="space-y-4 md:space-y-6 max-w-2xl mx-auto">

        <h1 className="text-5xl sm:text-6xl md:text-8xl font-bold text-white leading-tight">
          Elariz Rajabov
        </h1>

        <h2 className="text-3xl sm:text-4xl md:text-6xl font-bold text-blue-500">
          Frontend & Fullstack Developer
        </h2>

        <p className="text-white/50 text-base md:text-xl leading-relaxed pt-2">
          Fullstack Developer based in Baku.
          I build high-performance web and desktop applications with a focus on UI design, scalable architecture, and seamless maintenance.
        </p>

        <div className="pt-4 md:pt-6 relative">
          
          <button
            onClick={() => setOpen(!open)}
            className="bg-blue-600 hover:bg-blue-700 text-white px-6 md:px-8 py-3 md:py-4 rounded-full text-sm md:text-base transition-colors"
          >
            Get in Touch
          </button>

         
          {open && (
            <>
            
              <div
                className="fixed inset-0 z-10"
                onClick={() => setOpen(false)}
              />

              <div className="absolute left-0 mt-3 z-20 bg-[#1a1a2e] border border-white/10 rounded-2xl shadow-2xl p-2 min-w-[220px] flex flex-col gap-1">

               
                <Link
                  href="mailto:elarizreceb@gmail.com"
                  onClick={() => setOpen(false)}
                  className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-white/10 transition-colors text-white text-sm"
                >
                  <span className="flex items-center justify-center w-8 h-8 rounded-full bg-blue-600/20">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#3b82f6" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <rect width="20" height="16" x="2" y="4" rx="2"/>
                      <path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/>
                    </svg>
                  </span>
                  <div>
                    <p className="font-medium">Email</p>
                    <p className="text-white/40 text-xs">elarizreceb@gmail.com</p>
                  </div>
                </Link>

                
                <Link
                  href="https://wa.me/994505083281"
                  target="_blank"
                  rel="noopener noreferrer"
                  onClick={() => setOpen(false)}
                  className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-white/10 transition-colors text-white text-sm"
                >
                  <span className="flex items-center justify-center w-8 h-8 rounded-full bg-green-600/20">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="#22c55e">
                      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                    </svg>
                  </span>
                  <div>
                    <p className="font-medium">WhatsApp</p>
                    <p className="text-white/40 text-xs">+994 50 508 32 81</p>
                  </div>
                </Link>

              </div>
            </>
          )}
        </div>

      </div>
    </section>
  )
}